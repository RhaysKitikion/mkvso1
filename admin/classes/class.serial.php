...
//			trigger_error("You must close your device before to set an other one", E_USER_WARNING);
	...
	function SaveParams()
	{
		$this->_exec("stty -F " . $this->_device . " -g", $out);
		$this->_params=$out[0];
	}
	function RestoreParams()
	{
		$this->_exec("stty -F " . $this->_device . " ".$this->_params);	
	}
	...
	function ValidatorSetOptions ()
	{
		if ($this->_dState !== SERIAL_DEVICE_SET)
		{
			trigger_error("Unable to set validator options port : the device is either not set or opened", E_USER_WARNING);
			return false;
		}
		...
		if ($this->_os === "linux")
		{
			$ret = $this->_exec("stty -F " . $this->_device . " clocal cread -cstopb -parenb cs8  -ignbrk -brkint -parmrk -istrip -inlcr -icrnl -ixon -opost -echo -echonl -icanon -isig -iexten", $out);
		}
		else
		{
//			$ret = $this->_exec("mode " . $this->_windevice . " STOP=" . $length, $out);
